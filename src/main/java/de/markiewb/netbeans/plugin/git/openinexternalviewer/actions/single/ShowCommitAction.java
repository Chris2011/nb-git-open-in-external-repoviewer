...
@ActionID(
        category = "Git",
        id = "de.markiewb.netbeans.plugin.git.openinexternalviewer.actions.single.ShowCommitAction"
)
@ActionRegistration(
        displayName = "#CTL_ShowCommitAction"
)
@Messages("CTL_ShowCommitAction=Show commit (in browser)")
public final class ShowCommitAction extends AbstractSingleTypeAction {

    public ShowCommitAction(FileObject context) {
        super(context);
    }

    @Override
    protected RepoStrategy.Type getSupportedType() {
        return RepoStrategy.Type.SHOW_COMMIT;
    }
