...
@ActionID(
        category = "Git",
        id = "de.markiewb.netbeans.plugin.git.openinexternalviewer.actions.single.ShowDiffAction"
)
@ActionRegistration(
        displayName = "#CTL_ShowDiffAction"
)
@Messages("CTL_ShowDiffAction=Show diff (in browser)")
public final class ShowDiffAction extends AbstractSingleTypeAction {

    public ShowDiffAction(FileObject context) {
        super(context);
    }

    @Override
    protected RepoStrategy.Type getSupportedType() {
        return RepoStrategy.Type.SHOW_COMMITDIFF;
    }
