...
@ActionID(
        category = "Git",
        id = "de.markiewb.netbeans.plugin.git.openinexternalviewer.actions.single.ShowLogAction"
)
@ActionRegistration(
        displayName = "#CTL_ShowLogAction"
)
@Messages("CTL_ShowLogAction=Show log (in browser)")
public final class ShowLogAction extends AbstractSingleTypeAction {

    public ShowLogAction(FileObject context) {
        super(context);
    }

    @Override
    protected RepoStrategy.Type getSupportedType() {
        return RepoStrategy.Type.SHOW_LOG;
    }
